...
function scoreClass(type: string, score: number | null): string {
    ...
    if (!ctx) return 'text-slate-700';
    if (ctx === 'Severe' || ctx === 'Mod-Severe') return 'font-semibold text-red-700';
    if (ctx === 'Moderate') return 'font-semibold text-amber-700';
    ...
    return (
        ...
        >
            ...
            <td className="px-4 py-3 text-sm">
                ...
                {context === 'recent' && (
                    <span className="text-slate-600">{formatDate(item.completed_at)}</span>
                )}
            </td>
        </tr>
    );
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-4">
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className={`inline-flex items-center justify-center rounded-full w-5 h-5 text-[10px] font-bold bg-white bg-opacity-60 ${count > 0 ? 'text-inherit' : 'text-slate-400'}`}>
                        {count}
                    </span>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-100 text-sm">
                            <thead className="bg-slate-50">
                                <tr>
                                    <th className="px-4 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Participant</th>
                                    <th className="px-4 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Assessment</th>
                                    <th className="px-4 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Department</th>
                                    <th className="px-4 py-2.5 text-center text-xs font-semibold text-slate-500 uppercase tracking-wide">Score</th>
    ...
    return (
        <AppShell breadcrumbs={[{ label: 'Clinical' }, { label: 'Assessments Worklist' }]}>
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h1 className="text-xl font-bold text-slate-900">Assessments Worklist</h1>
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
                {[
                    { label: 'Overdue',      count: overdue.length,  color: 'bg-red-50 border-red-200 text-red-800' },
                    { label: 'Due This Week',count: dueSoon.length,  color: 'bg-amber-50 border-amber-200 text-amber-800' },
                    { label: 'Completed (30d)',count: recent.length, color: 'bg-green-50 border-green-200 text-green-800' },
                ].map(chip => (
            ...
            <Section
                ...
                items={overdue}
                context="overdue"
                headerClass="bg-red-50 text-red-800 border-b border-red-100"
            ...
            <Section
                ...
                items={dueSoon}
                context="due_soon"
                headerClass="bg-amber-50 text-amber-800 border-b border-amber-100"
            ...
            <Section
                ...
                items={recent}
                context="recent"
                headerClass="bg-green-50 text-green-800 border-b border-green-100"
