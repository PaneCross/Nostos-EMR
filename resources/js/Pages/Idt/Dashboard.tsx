        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" data-testid="schedule-meeting-modal">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    <h2 className="font-semibold text-slate-800">Schedule IDT Meeting</h2>
...
function MeetingCard({ meeting, showQueue = false }: { meeting: IdtMeeting; showQueue?: boolean }) {
    ...
    return (
        <div
            ...
            className={`rounded-xl border p-4 ${meeting.status === 'in_progress' ? 'border-amber-300 bg-amber-50/60' : 'border-slate-200 bg-white'}`}
        >
    ...
    return (
        <AppShell breadcrumbs={[{ label: 'IDT Dashboard' }]}>
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-xl font-bold text-slate-900">IDT Dashboard</h1>
                    <p className="text-sm text-slate-500 mt-0.5">Interdisciplinary Team meetings and participant review queue</p>
                </div>
                <button
