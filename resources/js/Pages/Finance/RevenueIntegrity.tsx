...
function KpiCard({
    ...
    return (
        <div className={`rounded-xl border p-5 ${alert && value > 0 ? 'border-amber-300 bg-amber-50 dark:bg-amber-950/60' : 'border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
            <div className="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wide">{label}</div>
            <div className={`text-2xl font-bold mt-1 ${alert && value > 0 ? 'text-amber-700 dark:text-amber-300' : 'text-gray-900 dark:text-slate-100'}`}>
                {displayValue}
            </div>
        ...
        <AppShell>
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Revenue Integrity Dashboard</h1>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            onClick={refresh}
                            disabled={loading}
                            className="px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/50 disabled:opacity-50"
                        >
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-3">
                        <div>
                            <h2 className="text-base font-semibold text-gray-800 dark:text-slate-200">HCC Coding Opportunities</h2>
                        ...
                        {gaps.length > 0 && (
                            <div className="text-right">
                                <div className="text-sm font-medium text-gray-700 dark:text-slate-300">Est. Monthly Impact</div>
                                ...
                                {gaps.length === 0 ? (
                                    ...
                                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                        <td className="px-3 py-2 font-medium text-gray-900 dark:text-slate-100">
                                            ...
                                            <div className="text-xs text-gray-400 dark:text-slate-500">{gap.mrn}</div>
                                        </td>
                                        <td className="px-3 py-2 font-mono text-xs text-gray-700 dark:text-slate-300">{gap.icd10_code}</td>
                                        <td className="px-3 py-2">
                                            <div className="text-xs font-medium text-gray-800 dark:text-slate-200">HCC {gap.hcc_category}</div>
                                            <div className="text-xs text-gray-500 dark:text-slate-400">{gap.hcc_label}</div>
                                        </td>
                                        <td className="px-3 py-2 text-right text-green-700 dark:text-green-300 font-medium text-xs">
                ...
                <div>
                    <div className="mb-3">
                        <h2 className="text-base font-semibold text-gray-800 dark:text-slate-200">Encounters Missing Required Fields</h2>
                        <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                {pending.length === 0 ? (
                                    ...
                                    <tr key={enc.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                        <td className="px-3 py-2 font-medium text-gray-900 dark:text-slate-100">{enc.participant_name}</td>
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex flex-wrap gap-1">
                                                {enc.missing_fields.map(f => (
                                                    <span key={f} className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300 text-xs rounded">
                                                        {f.replace('_', ' ')}
                                                    </span>
                                                ))}
                                            </div>
